...
template<class S, class T>
Graphe<S, T>::Graphe() : prochaineCle(0), listeSommets(NULL), listeAretes(NULL) { }
...
{
	Sommet<T> * sommetCree = new Sommet<T>(prochaineCle++, info);
...
Arete<S, T>* Graphe<S, T>::creeArete(Sommet<T>* debut, Sommet<T>* fin, const S & info)
{
	...
	Arete<S, T> * nouvelleArete = new Arete<S, T>(prochaineCle++, debut, fin, info);
	listeAretes.push_back(nouvelleArete);
